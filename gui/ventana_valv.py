...
POS = {
    ...
    "sol": {
        "campo_presion_seguriad": (5,  48),
        "presion_manual_lbl": (45,  125), "btn_sol_toggle":   (185, 115),
    },
    "per": {
        "per1_lbl":   (35, 53),    "btn_per1":    (212,  48),
        "per2_lbl": (35,  127),     "btn_per2":    (212,  117),
...
class VentanaValv(tk.Frame):
    ...
    def _configurar_estilos(self):
        ...
        style.configure("AB.TButton", padding=(16, 8),
                        font=getattr(C, "FONT_BASE", ("Calibri", 16)))
        style.map("AB.TButton", background=[
                  ("!disabled", "#e6e6e6"), ("pressed", "#d0d0d0")])

        style.configure("ABSelected.TButton", padding=(16, 8), font=getattr(
            C, "FONT_BASE", ("Calibri", 16)), background="#bdbdbd")
        style.map("ABSelected.TButton", background=[
                  ("!disabled", "#bdbdbd"), ("pressed", "#9e9e9e")])

        RUN_COLOR = "#27ae60"
        STOP_COLOR = "#db4231"
        # boton de abrir/cerrar
        style.configure("AbrirBtn.TButton", padding=(16, 8),
                        font=getattr(C, "FONT_BASE", ("Calibri", 16)))
        style.map("AbrirBtn.TButton", background=[
                  ("!disabled", RUN_COLOR), ("active", RUN_COLOR), ("pressed", RUN_COLOR)])
        style.configure("CerrarBtn.TButton", padding=(16, 8),
                        font=getattr(C, "FONT_BASE", ("Calibri", 16)))
        style.map("CerrarBtn.TButton", background=[
    ...
    def _build_ui(self):
        ...
        # Contenedor derecho
        cont = ttk.Frame(self)
        # sin padding para aprovechar ancho
    ...
    def _crear_seccion_valv(self, parent, sec_id: str, titulo: str) -> ttk.LabelFrame:
        ...
        title_lbl = ttk.Label(frame, text=titulo, font=TITLE_FONT.get(
            sec_id, ("Calibri", 20, "bold")))
        title_x, title_y = TITLE_POS.get(sec_id, (10, 8))
        title_lbl.place(x=title_x, y=title_y)

        if sec_id == "v1":
            # --- Tarjeta: Válvula 1 (Entrada) ---
            self.btn_v1_a = TouchButton(
                frame, text="Posición A", style="AB.TButton", command=lambda: self._seleccionar_posicion("v1", "A"))
            self.btn_v1_a.place(
                x=POS[sec_id]["btn_v1_a"][0], y=POS[sec_id]["btn_v1_a"][1])

            self.btn_v1_b = TouchButton(
                frame, text="Posición B", style="AB.TButton", command=lambda: self._seleccionar_posicion("v1", "B"))
            self.btn_v1_b.place(
                x=POS[sec_id]["btn_v1_b"][0], y=POS[sec_id]["btn_v1_b"][1])

        elif sec_id == "v2":
            # --- Tarjeta: Válvula 2 (Salida) ---
            self.btn_v2_a = TouchButton(
                frame, text="Posición A", style="AB.TButton", command=lambda: self._seleccionar_posicion("v2", "A"))
            self.btn_v2_a.place(
                x=POS[sec_id]["btn_v2_a"][0], y=POS[sec_id]["btn_v2_a"][1])

            self.btn_v2_b = TouchButton(
                frame, text="Posición B", style="AB.TButton", command=lambda: self._seleccionar_posicion("v2", "B"))
            self.btn_v2_b.place(
                x=POS[sec_id]["btn_v2_b"][0], y=POS[sec_id]["btn_v2_b"][1])

        elif sec_id == "con":
            # --- Tarjeta: Conexión equipo 2 ---
            self.btn_con_eq2 = TouchButton(frame, text=self._texto_conexion(
            ), style="AB.TButton", command=self._toggle_conexion)
            self.btn_con_eq2.place(
                x=POS[sec_id]["btn_con_eq2"][0], y=POS[sec_id]["btn_con_eq2"][1])

            self.btn_info_con = TouchButton(frame, text="?")
            self.btn_info_con.place(
                x=POS[sec_id]["btn_info_con"][0], y=POS[sec_id]["btn_info_con"][1])
            ...
            # --- Tarjeta: Bypass ---
            self.btn_bypass = TouchButton(frame, text=self._texto_bypass(
            ), style="AB.TButton", command=self._toggle_bypass)
            self.btn_bypass.place(
                x=POS[sec_id]["btn_bypass"][0], y=POS[sec_id]["btn_bypass"][1])

            self.btn_info_byp = TouchButton(frame, text="?")
            self.btn_info_byp.place(
                x=POS[sec_id]["btn_info_byp"][0], y=POS[sec_id]["btn_info_byp"][1])
            ...
            self.btn_sol_toggle = TouchButton(
                frame, text=self._texto_sol(), style=initial_style, command=self._toggle_sol)
            self.btn_sol_toggle.place(
                x=POS[sec_id]["btn_sol_toggle"][0], y=POS[sec_id]["btn_sol_toggle"][1])

            presion_manual_lbl = ttk.Label(
                frame, text="Modo manual:", font=getattr(C, "FONT_BASE", ("Calibri", 14)))
            presion_manual_lbl.place(
                x=POS[sec_id]["presion_manual_lbl"][0], y=POS[sec_id]["presion_manual_lbl"][1])
            ...
            campo_presion_seguriad.place(
                x=POS[sec_id]["campo_presion_seguriad"][0], y=POS[sec_id]["campo_presion_seguriad"][1])
            ...
            per1_lbl = ttk.Label(frame, text="Bomba 1 (BP1):", font=getattr(
                C, "FONT_BASE", ("Calibri", 14)))
            per1_lbl.place(x=POS[sec_id]["per1_lbl"][0],
                           y=POS[sec_id]["per1_lbl"][1])

            self.btn_per1 = TouchButton(frame, text=self._texto_per1(
            ), style="AB.TButton", command=self._toggle_per1)
            self.btn_per1.place(
                x=POS[sec_id]["btn_per1"][0], y=POS[sec_id]["btn_per1"][1])

            per2_lbl = ttk.Label(frame, text="Bomba 2 (BP2):", font=getattr(
                C, "FONT_BASE", ("Calibri", 14)))
            per2_lbl.place(x=POS[sec_id]["per2_lbl"][0],
                           y=POS[sec_id]["per2_lbl"][1])

            self.btn_per2 = TouchButton(frame, text=self._texto_per2(
            ), style="AB.TButton", command=self._toggle_per2)
            self.btn_per2.place(
                x=POS[sec_id]["btn_per2"][0], y=POS[sec_id]["btn_per2"][1])
    ...
    def _seleccionar_posicion(self, cual: str, pos: str):
        ...
        if cual == "v1":
            ...
            self.v1_pos.set(pos)
            self._refrescar_botones("v1")
            mensaje = f"$;3;1;8;{pos_code};!" if self.conexion_equipo2.get(
            ) else f"$;3;1;1;{pos_code};!"
