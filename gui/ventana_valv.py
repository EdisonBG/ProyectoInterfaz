...
class VentanaValv(tk.Frame):
    ...
    def _configurar_estilos(self):
        ...
        st.configure("Valv.TLabel", background=self._BG, foreground=self._TEXT)
        st.configure(
            "Valv.TEntry",
        ...
        st.configure("Valv.TButton",
                     padding=(8, 6), relief="raised", borderwidth=2,
        ...
        # A/B
        st.configure("AB.TButton", padding=(8, 6), relief="raised", borderwidth=2,
                     background=self._SURFACE, foreground=self._TEXT)
        st.map("AB.TButton", background=[("active", self._BORDER)])
        st.configure("ABSelected.TButton", padding=(8, 6), relief="raised", borderwidth=2,
                     background=self._PRIMARY, foreground="white")
        st.map("ABSelected.TButton", background=[("active", self._PRIMARY_ACTIVE)])
    ...
    def _card(self, parent, title, row, col, colspan=1):
        ...
        tk.Label(outer, text=title, bg=self._BG, fg=self._TEXT,
                 ...
                 anchor="w", padx=8, pady=2).grid(row=1, column=0, sticky="ew")
    ...
    def crear_widgets(self):
        ...
        # Válvula 1
        sec_v1 = self._card(panel, "Válvula 1 (Entrada)", 0, 0)
        ...
        self.btn_v1_a.grid(row=0, column=0, padx=6, pady=4, sticky="w")
        self.btn_v1_b.grid(row=0, column=1, padx=6, pady=4, sticky="e")

        # Válvula 2
        sec_v2 = self._card(panel, "Válvula 2 (Salida)", 0, 1)
        ...
        self.btn_v2_a.grid(row=0, column=0, padx=6, pady=4, sticky="w")
        self.btn_v2_b.grid(row=0, column=1, padx=6, pady=4, sticky="e")

        # Conexión equipo 2
        sec_con = self._card(panel, "Conexión equipo 2", 1, 0, colspan=2)
        self.btn_con_eq2 = ttk.Button(sec_con, text=self._texto_conexion(),
                                      style="ValvPrimary.TButton", command=self._toggle_conexion)
        self.btn_con_eq2.grid(row=0, column=0, padx=6, pady=4, sticky="w")

        # Válvula motor 1
        sec_vm1 = self._card(panel, "Válvula motor 1", 2, 0)
        ...
        self.btn_vm1_izq.grid(row=0, column=0, padx=6, pady=4, sticky="w")
        self.btn_vm1_der.grid(row=0, column=1, padx=6, pady=4, sticky="e")

        # Válvula motor 2
        sec_vm2 = self._card(panel, "Válvula motor 2", 2, 1)
        ...
        self.btn_vm2_izq.grid(row=0, column=0, padx=6, pady=4, sticky="w")
        self.btn_vm2_der.grid(row=0, column=1, padx=6, pady=4, sticky="e")

        # Solenoide
        sec_sol = self._card(panel, "Válvula solenoide (seguridad)", 3, 0, colspan=2)
        ...
        self.btn_sol_toggle.grid(row=0, column=0, columnspan=2, padx=6, pady=4, sticky="w")

        tk.Label(sec_sol, text="Presión de seguridad (bar):",
                 bg=self._SURFACE, fg=self._TEXT).grid(row=1, column=0, padx=6, pady=4, sticky="e")
        self.entry_p_seg = ttk.Entry(sec_sol, width=8, style="Valv.TEntry")
        self.entry_p_seg.grid(row=1, column=1, padx=6, pady=4, sticky="w")
        ...
        # Bombas
        sec_per = self._card(panel, "Bombas peristálticas", 4, 0, colspan=2)
        ...
        self.btn_per1.grid(row=0, column=0, padx=6, pady=(4, 2), sticky="w")
        self.btn_per2.grid(row=1, column=0, padx=6, pady=(2, 4), sticky="w")

        self._aplicar_estado_conexion()

    # ===== Persistencia/Handlers (sin cambios de lógica) =====
    def _cargar_posiciones(self):
        if not os.path.exists(self._pos_file): return
        try:
            with open(self._pos_file, newline="", encoding="utf-8") as f:
                for nombre, pos in csv.reader(f):
                    pos = (pos or "").strip().upper()
                    if nombre == "V1" and pos in ("A", "B"): self.v1_pos.set(pos)
                    elif nombre == "V2" and pos in ("A", "B"): self.v2_pos.set(pos)
    ...
    def _texto_sol(self): return "Cerrar válvula" if self.sol_abierta.get() else "Abrir válvula"
    def _texto_per1(self): return "Peristáltica 1: OFF → ON" if not self.per1_on.get() else "Peristáltica 1: ON → OFF"
    def _texto_per2(self): return "Peristáltica 2: OFF → ON" if not self.per2_on.get() else "Peristáltica 2: ON → OFF"
    def _texto_conexion(self): return "Conexión equipo 2: OFF" if not self.conexion_equipo2.get() else "Conexión equipo 2: ON"

    def _seleccionar_posicion(self, cual: str, pos: str):
        if pos not in ("A", "B"): return
        pos_code = "1" if pos == "A" else "2"
        if cual == "v1":
            self.v1_pos.set(pos); self._refrescar_botones("v1")
            mensaje = f"$;3;1;8;{pos_code};!" if self.conexion_equipo2.get() else f"$;3;1;1;{pos_code};!"
            self._guardar_posiciones()
        elif cual == "v2":
            if self.conexion_equipo2.get(): return
            self.v2_pos.set(pos); self._refrescar_botones("v2")
            mensaje = f"$;3;2;1;{pos_code};!"; self._guardar_posiciones()
        else: return
    ...
    def _toggle_conexion(self):
        ...
        self._aplicar_estado_conexion()
        if nuevo and hasattr(self.controlador, "enviar_a_arduino"):
            self.controlador.enviar_a_arduino("$;3;0;8;!")
    ...
    def _leer_presion_float_capada(self, v) -> float:
        try:
            ...
            p = 20.0
        return round(min(p, 20.0), 1)

    def _aplicar_presion_y_enviar_auto(self, valor):
        p = self._leer_presion_float_capada(valor)
        self.sol_presion = p
        self.entry_p_seg.delete(0, tk.END); self.entry_p_seg.insert(0, f"{p:.1f}")
        if hasattr(self.controlador, "enviar_a_arduino"):
            self.controlador.enviar_a_arduino(f"$;3;5;0;{int(round(p*10))};!")

    def _toggle_sol(self):
        p = self._leer_presion_float_capada(self.entry_p_seg.get())
        self.sol_presion = p
        self.entry_p_seg.delete(0, tk.END); self.entry_p_seg.insert(0, f"{p:.1f}")
        nuevo = not self.sol_abierta.get(); self.sol_abierta.set(nuevo)
        self.btn_sol_toggle.configure(text=self._texto_sol())
        if hasattr(self.controlador, "enviar_a_arduino"):
            estado = "1" if nuevo else "2"
            self.controlador.enviar_a_arduino(f"$;3;5;1;{estado};{int(round(p*10))};!")

    def _seleccionar_motor(self, cual: str, dir_code: str):
        if cual not in ("vm1", "vm2") or dir_code not in ("D", "I"): return
        if cual == "vm1": self.vm1_dir.set(dir_code); self._refrescar_botones_vm("vm1"); motor = "3"
        else:             self.vm2_dir.set(dir_code); self._refrescar_botones_vm("vm2"); motor = "4"
        val = "1" if dir_code == "D" else "2"
        if hasattr(self.controlador, "enviar_a_arduino"):
            self.controlador.enviar_a_arduino(f"$;3;{motor};1;{val};!")

    def _toggle_per1(self):
        nuevo = not self.per1_on.get(); self.per1_on.set(nuevo)
        self.btn_per1.configure(text=self._texto_per1())
        if hasattr(self.controlador, "enviar_a_arduino"):
            self.controlador.enviar_a_arduino(f"$;3;6;1;{'1' if nuevo else '2'};!")

    def _toggle_per2(self):
        nuevo = not self.per2_on.get(); self.per2_on.set(nuevo)
        self.btn_per2.configure(text=self._texto_per2())
        if hasattr(self.controlador, "enviar_a_arduino"):
            self.controlador.enviar_a_arduino(f"$;3;7;1;{'1' if nuevo else '2'};!")
