...
class VentanaValv(tk.Frame):
    """Panel de control de válvulas, solenoide de seguridad y bombas peristálticas."""
    ...
    def __init__(self, master, controlador, arduino, *args, **kwargs):
        ...
        # Archivo de persistencia (CSV plano clave,valor)
        self._pos_file = os.path.join(os.path.dirname(__file__), "valv_pos.csv")

        # Estados V1/V2 (persisten entre sesiones)
        self.v1_pos = tk.StringVar(value="A")  # "A" o "B"
        self.v2_pos = tk.StringVar(value="A")  # "A" o "B"
        ...
        # Bypass (1 o 2), persiste en CSV con clave BYP
        self.bypass_sel = tk.IntVar(value=1)

        # Construcción de UI
        self._construir_ui()

        # Cargar estado persistido y reflejar UI
    ...
    def _construir_ui(self) -> None:
        # Rejilla raíz: barra izquierda fija + contenedor derecho expandible
        ...
        # --- Tarjeta: Válvula solenoide (seguridad) ---
    ...
    # ------------------------------------------------------------------
    def _cargar_posiciones(self) -> None:
    ...
    def _guardar_posiciones(self) -> None:
        try:
    ...
    def _seleccionar_posicion(self, cual: str, pos: str) -> None:
        ...
        if cual == "v1":
            ...
            if self.conexion_equipo2.get():
                return  # V2 deshabilitada cuando conexión eq.2 está activa
    ...
    def _toggle_bypass(self) -> None:
        ...
        # Notificar a la ventana MFC para recalcular % de mezcla y sincronizar gases si aplica
